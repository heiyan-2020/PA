  ...
  if (!make_token(e)) {
    *success = false;
	...
	for(int i = 0; i < nr_token; i ++) {
		...
		}else if((tokens[i].type == TK_SUB && (i == 0 || tokens[i - 1].type  == TK_AND || tokens[i - 1].type  == TK_NEQ ||tokens[i - 1].type  == TK_EQ ||tokens[i - 1].type == TK_DIV ||tokens[i - 1].type  == TK_MUL || tokens[i - 1].type  ==  TK_SUB ||tokens[i - 1].type  == TK_PLUS ||tokens[i - 1].type  == TK_LEFT || tokens[i - 1].type == TK_DEREF))) {
